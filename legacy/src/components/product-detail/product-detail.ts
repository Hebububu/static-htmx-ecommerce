		...
		if (variantSelect) {
			...
			variantSelect.addEventListener('change', async () => {
				if (!hasMainRow(container)) {
					...
					container
						.querySelector<HTMLElement>('#pd-storage')
	...
	filterBtns.forEach((btn) => {
		btn.addEventListener('click', () => {
			...
			filterBtns.forEach((b) =>
				b.classList.toggle('product-review-filter__btn--active', b === btn)
			);
			...
			reviewItems.forEach((item) => {
				if (filter === 'photo') {
					item.classList.toggle(
						...
						!item.hasAttribute('data-has-photo')
					);
				} else {
	...
	if (key.startsWith('addon-')) {
		...
		if (!remaining?.length) {
			const card = container.querySelector<HTMLElement>(
				`.product-detail__addon[data-addon-index="${index}"]`
			);
...
function updateTotal(container: HTMLElement): void {
	...
	container.querySelectorAll<HTMLElement>('.product-detail__sel-row').forEach((row) => {
		...
		const qty = Number(
			row.querySelector<HTMLElement>('.product-detail__qty-value')?.textContent ?? 0
		);
		total += unitPrice * qty;
	});
